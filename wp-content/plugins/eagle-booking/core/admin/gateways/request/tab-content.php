<?php if ( eb_get_option('eagle_booking_payment_method')['request'] ) : ?>
  <div id="eagle_booking_checkout_payment_request_tab">
      <p class="checkout-mssg"><?php echo do_shortcode(eb_get_option('eagle_booking_request_mssg')) ?></p>
      <form action="<?php echo eb_checkout_page() ?>" method="post">
          <input type="hidden" name="eagle_booking_payment_method" value="booking_request">
          ...
          <input type="hidden" name="eagle_booking_form_action_type" value="booking_request">
          <input type="hidden" name="eagle_booking_form_payment_status" value="Pending">
          <button type="submit" class="btn eb-btn btn-request"><?php echo esc_html__('Send Booking Request','eagle-booking') ?></button>
      </form>
  </div>
<?php endif ?>
