<?php
if(!class_exists("ZanteBase")) {
	class ZanteBase {
    	public $key = "2AF882CA00FCD528";
    	private $product_id = "1";
    	private $product_base = "zante";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->hasCheckUpdate) {
				if(function_exists("add_action")){
					add_action( 'admin_post_zante_fupc', function(){
					...
					if($this->isTheme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=zante_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
